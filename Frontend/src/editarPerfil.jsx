    ...
    try {
      ...
      if (avatar && avatar !== user.avatar && avatar instanceof File) {
        ...
        const res = await fetch(
          `http://localhost:3001/usuarios/avatar/${user.id_usuario}`,
          {
            method: "POST",
            body: formData,
          }
      ...
      const response = await fetch(
        `http://localhost:3001/usuarios/editar/${user.id_usuario}`,
        {
          method: "PUT",
      ...
      >
        ...
        <div className={`${RegistroCSS.col} ${RegistroCSS["col-der"]}`}>
          <div className={RegistroCSS["regresar-titulo"]}>
            <Link
              to="/perfilUsuario"
              className={`${RegistroCSS["regresar-elemento"]} ${RegistroCSS["boton-pixel-corners"]}`}
            >
              <img
